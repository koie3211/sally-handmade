            ...
            Alpine.data('addTransaction', (categories) => ({
                ...
                dragEnd() {
                    ...
                    if (this.dragY > 100) {
                        ...
                        setTimeout(() => { this.open = false; this.dragY = 0 }, 280)
                    } else {
                        this.dragY = 0
                    }
                },
                ...
                parseInvoiceQR(text) {
                    // 台灣電子發票左側 QR Code（77碼固定頭）：
                    // [0-9]發票號碼 [10-16]民國日期YYYMMDD [21-28]未稅hex [29-36]含稅hex
                    // [77+] :[編碼]:[品項數]:[品名1(base64)]:...
                    if (!text || text.length < 37) return false
                    if (!/^[A-Z]{2}\d{8}$/.test(text.slice(0, 10))) return false
                    try {
                        const dateStr = text.slice(10, 17)
                        const rocYear = parseInt(dateStr.slice(0, 3), 10)
                        this.date = `${rocYear + 1911}-${dateStr.slice(3, 5)}-${dateStr.slice(5, 7)}`

                        const total = parseInt(text.slice(29, 37), 16)
                        if (!isNaN(total) && total > 0) this.amount = total.toString()
...
<nav class="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 nav-bar z-40">
    <div class="flex items-start justify-around pt-2 px-2">
        <a href="{{ route('budget.dashboard') }}"
           class="flex flex-col items-center gap-0.5 px-2 py-1 rounded-xl transition-colors
        ...
        <a href="{{ route('budget.calendar') }}"
           class="flex flex-col items-center gap-0.5 px-2 py-1 rounded-xl transition-colors
                  {{ request()->routeIs('budget.calendar') ? 'text-indigo-600' : 'text-slate-400' }}">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
            </svg>
            <span class="text-xs font-medium">行事曆</span>
        </a>
